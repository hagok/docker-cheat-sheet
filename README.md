...
дальше пока про гит, в процессе формирования про докер
...
### git reset
`git reset` - используется для отмены изменений в репозитории. Она позволяет перемещать указатели HEAD и текущей ветки, а также управлять состоянием индекса (staging area) и рабочей директории
```
git reset --soft HEAD~1   # отмена последнего коммита, изменения остаются в staged 
git reset --mixed HEAD~1  # отмена последнего коммита, изменения в рабочей папке, но не staged
git reset --hard HEAD~1   # полная отмена последнего коммита, все изменения удалены
```
<br />

### git revert
`git revert` - отменяет изменения, внесённые указанным коммитом, создавая новый коммит с обратными изменениями. В отличие от `git reset`, она не удаляет историю, что делает её безопасной для использования в общих репозиториях.
```
git revert HEAD           # отмена последнего коммита
git revert a1b2c3d        # отмена конкретного коммита по хешу
git revert -m 1 789def    # отмена слияния (merge commit)

git revert -n abc123      # изменения добавлены в индекс, но коммита нет
git commit -m "Отмена коммита abc123"
```
<br />

### git stash
`git stash` - это команда в системе контроля версий Git, которая позволяет временно сохранить изменения в рабочем каталоге, чтобы можно было переключиться на другую ветку или выполнить другие команды без необходимости коммитить эти изменения
```
git stash          # спрятать изменения  
git stash pop      # вернуть изменения 
git stash list     # все записи с меткой времени
```
<br />
...
```
Теперь можно писать git lol для красивого вывода логов!
<br/>
